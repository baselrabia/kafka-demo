...
func consumeMessage(msg *sarama.ConsumerMessage) {
    ...
    if err != nil {
        log.Print("Error Parsing the Kafka Message : ", err)
		return 
    }

	mu.Lock()
	
	 if name, ok := sliceMap[kmsg.ID]; ok {
		// log.Printf("ID %d: %s & %s", msg.ID, msg.Name, name)
		log.Printf("ID %d: %s & %s", kmsg.ID, name, kmsg.Name)
		delete(sliceMap, kmsg.ID)  
 	}else {
		// save the name of the msg id if it never exist
		sliceMap[kmsg.ID] = kmsg.Name
	}
